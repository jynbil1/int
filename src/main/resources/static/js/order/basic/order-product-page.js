...
$(document).ready(function () {
    ...
    getOrderListCount(searchKeyword, banjjakOrder);
    ...
    $("#recordDataOrder").on("click", function () {
        recordOrder = $('#recordDataOrder').is(":checked") ? 1 : 0;
        if (recordOrder === 1) {
            $('td[name="order-date"]').show();
            $('.checkbox-table thead tr td:nth-child(1)').show();
        } else {
            $('td[name="order-date"]').hide();
            $('.checkbox-table thead tr td:nth-child(1)').hide();
        }
    });
    ...
    $('#submitOrder').on('click', function (event) {
        if ($(".selected").length > 0) {
            ...
            $(".selected").each(function () {
                ...
                var manager_id = $(this).find("[name='manager-id']").text();
                var manager_email = $(this).find("[name='manager-email']").text();
                var cc_email = $(this).find("[name='cc-email']").text();
                var hanpoom_PIC = $(this).find("[name='hanpoom-pic']").text();
                var pic_phone_number = $(this).find("[name='pic-phone-number']").text();
...
function getOrderList(page, item_range, keyword, sort, order, banjjak_order) {
    $('.loading').css('display', 'table');
    $.ajax({
        url: '/orderProductList?page=' + page + '&item_range=' + item_range + '&keyword=' + keyword + '&sort=' + sort + '&order=' + order + '&banjjak_order=' + banjjak_order,
        type: 'GET',
            ...
            if (response.code == '200') {
                ...
                $.each(jsonObject, function (i, object) {
                    ...
                    tr += '<tr>' +
                        ...
                        '<td name="manager-id" style="display: none;">' + object.manager_id + '</td>' +
                        '<td name="manager-name">' + object.manager_name + '</td>' +
                        '<td name="manager-rank">' + object.manager_rank + '</td>' +
                        '<td name="manager-email">' + object.manager_email + '</td>' +
                        '<td name="manager-number">' + object.phone_number + '</td>' +
                        '<td name="cc-email" style="display: none;">' + object.cc_email + '</td>' +
                        '<td name="hanpoom-pic" style="display: none;">' + object.hanpoom_PIC + '</td>' +
                        '<td name="pic-phone-number" style="display: none;">' + object.pic_phone_number + '</td>' +
...
function getOrderListCount(keyword, banjjak_order) {
    $.ajax({
        url: '/getOrderProductCount?keyword=' + keyword + '&banjjak_order=' + banjjak_order,
        type: 'GET',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
...
function createPagination(current, total) {
    ...
    getOrderList(currentPage, itemRange, searchKeyword, sortBy, orderBy, banjjakOrder);
    currentPage = current;
