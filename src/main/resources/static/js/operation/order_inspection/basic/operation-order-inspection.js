    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            var tableDetails = '<tr> <td class="td-order-no"> #' + order_no + '</td><td class="td-order-date" colspan="3"><span>주문 일자: <span class="date-format">' + date + ' ' + time + '</span></span></td>' +
                ...
                '<button type="submit" class="cancelScannedProduct button primary">스캔 취소</button><button type="submit" class="cancelScannedProductActive button greyed">스캔 취소</button>' +
                '<button type="submit" class="saveScannedProducts button primary" data-order-no="' + order_no + '">저장</button>' +
                '<button type="submit" id="cancelInspection" class="button greyed">검수 취소 </button></td></tr>'

            $('tbody#tableDetails').html(tableDetails);

            var productNoInputDiv = '<p class="warning-msg"><span>⚠</span> 모든 상품이 아직 검수되지 않았습니다. 다시 확인해주세요.</p><input type="text" id="productNoInput" name="i_product_no" autofocus placeholder="상품 검수 | 상품 바코드를 입력하세요."><span class="material-icons-outlined"> document_scanner </span> <input type="text" id="productNoInputCancel" name="i_product_no_cancel" autofocus placeholder="상품 검수 | 검수 취소할 상품 바코드를 입력하세요."><span class="material-icons-outlined"> document_scanner </span>';
...
function scanAndIncrementCount(i_product_barcode, order_no) {
    ...
    if (i_product_barcode === order_no && row === row_green) {
        ...
        $('#orderInspectionList tr').each(function () {
            ...
            if (input_pc > input_qty) {
                $(this).addClass("row-red");
                $(this).removeClass("row-green");
                $(this).prependTo("#orderInspectionList")
            } else if (input_pc === input_qty) {
                $(this).addClass("row-green");
                $(this).removeClass("row-red");
                $(this).appendTo("#orderInspectionList")
            } else {
                $(this).removeClass("row-green");
                $(this).removeClass("row-red");
            }
